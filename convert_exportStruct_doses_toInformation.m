function [SigFeaturesArray,dosestruct] = convert_exportStruct_doses_toInformation(fileDateName,parameters)
close all
...
%function to exract the cell traces, normalized and not
    ...
    basalLength = 3; %choose basal length
        [smadCellTracesNorm,smadCellTracesDiff,smadCellTraces] = extractTraces(exportStruct,indices,smadTracesString,finalFrame,stimulationFrame,basalLength);
        [reporterCellTracesNorm,reporterCellTracesDiff,reporterCellTraces] = extractTraces(exportStruct,indices,reporterTracesString,finalFrame,stimulationFrame,basalLength);


    basalValues = reporterCellTraces(:,stimulationFrame-basalLength:stimulationFrame);
    plottingMat = reporterCellTraces./nanmedian(basalValues(:)); %normalize to prestimulus median value
    plottingMatNorm = reporterCellTracesNorm;
    plottingMatDiff = reporterCellTracesDiff;
    
    plottingMatInt = nan(size(plottingMat));
    plottingMatIntNorm = nan(size(plottingMat));
    for ijk = stimulationFrame:size(plottingMat,2)
        plottingMatInt(:,ijk) = sum(plottingMat(:,stimulationFrame:ijk),2);
        plottingMatIntNorm(:,ijk) = sum(plottingMatNorm(:,stimulationFrame:ijk),2);
    end
...
for i=1:length(doseList)
    ...
    response.abs = plottingMat(idx,:);
    response.fc = plottingMatNorm(idx,:);
    response.diff = plottingMatDiff(idx,:);
    response.int = plottingMatInt(idx,:);
    response.fcint = plottingMatIntNorm(idx,:);

        responseMatrix = response.(parameters.scalartype);
...
function [cellTracesNorm,cellTracesDiff,cellTraces] = extractTraces(exportStruct,indices,xTracesString,finalFrame,stimulationFrame,basalLength)
...
cellTracesDiff = cellTraces-basalMatrix';
end