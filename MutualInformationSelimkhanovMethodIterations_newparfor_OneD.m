...
function [] = MutualInformationSelimkhanovMethodIterations_newparfor_OneD(fileDateName,parameters)
k=3;
...
%loads the specific file you want
    ...
    [SigFeaturesArray,dosestruct] = convert_exportStruct_doses_toInformation(fileDateName,parameters);
...
%     selectedFeatures = [selectedFeatures selectedFeatures+pmatdim2];
    ...
    %NEXT BEST
    tpoints = parameters.tpoints;
%     selectedFeaturesVec = [stimulationFrame+tpoints(1):stimulationFrame+tpoints(2)];
    selectedFeaturesVec = tpoints + stimulationFrame+1;

    
    dimensions=1;
    nInputSignalConditionsInStruct = length(SigFeaturesArray);
    featuresVector = selectedFeaturesVec;
    assembledFeatures1 = chooseQFfunct(SigFeaturesArray,featuresVector,dimensions); %assembles an array of 1d ([x]), 2d ([x,y]), or 3d ([x,y,z]) timepoint combinations depedning on the dimension input
    
    featuresVector  = selectedFeaturesVec + pmatdim2;
    assembledFeatures2 = chooseQFfunct(SigFeaturesArray,featuresVector,dimensions); %assembles an array of 1d ([x]), 2d ([x,y]), or 3d ([x,y,z]) timepoint combinations depedning on the dimension input
    
 % you do not need fold change data when doing 2 dimensional information.
%     assembledFeatures = [assembledFeatures1 assembledFeatures2];
%     selectedFeatures = [selectedFeaturesVec selectedFeaturesVec+pmatdim2];

% use only the data from the absolute traces, since fold chagne data come
% sfrom two dimensional coupling of t<=0 and t>>0 time points from absolute
% traces
    assembledFeatures = assembledFeatures1;
    selectedFeatures = selectedFeaturesVec;
...
%probability distributions
    smthness=2000;
    number_of_PofS = parameters.number_of_PofS; %number of probability distributions to test
    lowsigma=0.0001;
    highsigma=1;
    ...
    for j = 2:size(InputSignals,2)-1
        oneoneone = zeros(size(InputSignals(1,:)));
        oneoneone([1 j length(oneoneone)]) = 1;
        InputSignals(end+1,:) = oneoneone./sum(oneoneone);
    end
    number_of_PofS = size(InputSignals,1);
    %%%%%%%%%%%
...
%     percentageofsamplesize = 0.6 : 0.1 : 0.7;
    percentageofsamplesize = parameters.percentageofsamplesize;
    ...
    for iter = 1:iterations
        ...
        for samplingcycle = 1:length(percentageofsamplesize)
            ...
            %             bootresponsesGivenSvector = bootstrp(1,@(x) x,responsesGivenSvector); %cannot perform bootstrap because you end up with duplicate values and an information of -INF
                ...
                parfor inputcyclenum = 1:size(InputSignals,1)
                    q = InputSignals(inputcyclenum,:); %vector where each number is a probability of a give signal (condition) 
    ...
    cd(mfiledir)
    save(strcat(parameters.scalartype,'highINFORMATIONzSELIMKHANOV',datename,'-',num2str(dimensions),'-',num2str(iterations),'.mat'),'-v7.3');
...
Epsarray = cell(m,m,lengthOfFeatures);  
for ff=1:lengthOfFeatures
    for i = 1:m 
...
entropydiffR = zeros(1,lengthOfFeatures);  
for ff=1:lengthOfFeatures
