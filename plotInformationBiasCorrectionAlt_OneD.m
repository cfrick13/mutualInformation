function plotInformationBiasCorrectionAlt_OneD(fileDateName,dispName,parameters,numberOfPlots,plotNum)
...
%load the file
    cd(mfiledir)
    flist = dir(strcat(parameters.scalartype,'biasCorrMaxInfo*',datename,'*',num2str(parameters.iterations),'*.mat'));
...
tvecz1array = tvecz(t1array);
uTvals = tvecz1array;
timeStrArray = sortAssem1;
...
for i = 1:length(tvecz1array)
    ...
    a = find(uTvals == t1val);
%     b = find(uTvals == t2val);
    infoimage(a,1) = infoval;
    infoimagestd(a,1) = ebstdval;
...
f.Position = [158 537 1402 441];
    ...
    cyc2=0;
        ...
        stdvec = infoimagestd;
        xvec = uTvals;
        e = errorbar(xvec,infovec,stdvec);hold on
        e.Marker = mark;
        e.MarkerSize = 8;
        e.MarkerFaceColor = parameterz.color;
%         e.DisplayName = num2str(uTvals(i));
        e.DisplayName = parameters.scalartype;
        e.Color = parameterz.color./2;
        e.LineWidth =1;
    s.YLim =[0 1.5];
    s.XLim =[-20 180];
    s.Color = [0.9 0.9 0.9];
...
%     end
    tidx = tvecz1array==0;
        ...
        h=gca;
        h.XTick = 1:length(xvec);
        h.XTickLabel = timeStrArray;
